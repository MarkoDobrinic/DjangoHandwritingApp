...
from handwritingapp.utils.convert_coordinates import convert_coords, get_last_id
...
def fetch_data(request):
    ...
    if request.is_ajax():
        ...
        np.save(styles_dir + '/style-{}-chars.np'.format(file_id+1), np.array(bytes(input_text, 'utf-8')))
        np.save(styles_dir + '/style-{}-strokes.npy'.format(file_id+1), result)
